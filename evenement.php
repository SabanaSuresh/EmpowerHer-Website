...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["ajouter_favori"]) && isset($_SESSION["username"])) {
    ...
    if ($user) {
        $stmt = $pdo->prepare("INSERT INTO favoris (utilisateur_id, type_favori, id_favori) VALUES (?, 'evenement', ?)");
        $stmt->execute([$user['id'], $id]);

        header("Location: evenements.php?id=" . urlencode($id));
        exit();
    }
}

// recup les comms
$stmt = $pdo->prepare("SELECT c.*, u.username FROM commentaires_evenements c JOIN utilisateurs u ON c.user_id = u.id WHERE c.id_evenement = ? ORDER BY c.date_creation DESC");
$stmt->execute([$id]);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["commentaire"]) && !empty(trim($_POST["commentaire"]))) {
    ...
    $stmt = $pdo->prepare("INSERT INTO commentaires_evenements (id_evenement, user_id, contenu) VALUES (?, ?, ?)");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["supprimer_commentaire_id"]) && isset($_SESSION["username"]) && $_SESSION["username"] === "SabanaSuresh") {
    ...
    $stmt = $pdo->prepare("DELETE FROM commentaires_evenements WHERE id = ?");
...
<head>
    <meta charset="UTF-8">
    <title><?php echo htmlspecialchars($event["titre"]); ?> - EmpowHer</title>
    <style>
        ...
        .infos { text-align: right; font-size: 14px; margin-bottom: 30px; color: #444; }
        .description { font-size: 16px; margin-bottom: 20px; }
        .lien { font-style: italic; margin-bottom: 30px; }
...
<div class="main-content">
    <div class="titre"><?php echo htmlspecialchars($event["titre"]); ?></div>
    <div class="infos"><?php echo htmlspecialchars($event["annee"] . " – " . $event["lieu"]); ?></div>

    <div class="description"><?php echo nl2br(htmlspecialchars($event["description"])); ?></div>

    <div class="lien">Lien : <a href="<?php echo htmlspecialchars($event["lien"]); ?>" target="_blank"><?php echo htmlspecialchars($event["lien"]); ?></a></div>
            ...
            <?php foreach ($commentaires as $com): ?>
                <div class="comment-block">
                    <div class="comment-meta">
                        <?php echo htmlspecialchars($com['username']); ?> – <?php echo date('d/m/Y H:i', strtotime($com['date_creation'])); ?>
                    </div>
                    <div>
    ...
    <div class="footer-actions">
        ...
        <a href="https://twitter.com/intent/tweet?text=<?php echo urlencode($event["titre"] . ' - ' . $event["annee"]); ?>&url=<?php echo urlencode("https://tonsite.com/evenements.php?id=" . urlencode($id)); ?>" target="_blank">Partager sur X (Twitter)</a>
    </div>

    <div class="retour">
        ← <a href="frise.php">Retour à la frise chronologique</a>
    </div>
</div>
